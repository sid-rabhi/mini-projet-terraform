data "aws_ami" "my_ubuntu_ami" {
  ...
  filter {
    name   = "name"
    values = ["ubuntu-minimal/images/hvm-ssd/ubuntu-lunar-23.04-amd64-minimal-20231103"]
  }
}
...
resource "aws_instance" "mini-projet-ec2" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type        = "ssh"
      user        = var.user
      private_key = file("/home/kenshin/Documents/terraform-labs/keys/${var.ssh_key}.pem")
      host        = self.public_ip
    }
  }
